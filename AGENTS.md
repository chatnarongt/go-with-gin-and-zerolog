...
The API runs on `APP_PORT` (default `8080`). Configuration loads the env files passed by the command, then process environment variables override file values. The API command currently passes `.env` and `cmd/api/.env`; both paths must be readable because the loader returns an error for a missing file. `APP_LOG_LEVEL` defaults to zerolog `info` and accepts values `-1` through `5`. Other supported settings include `APP_ENV` (`development`, `staging`, `production`), `APP_DEBUG_MODE`, `APP_CORS_ENABLED` (default `true`), `APP_CORS_ALLOWED_ORIGINS` (default `*`), `APP_CORS_ALLOWED_METHODS`, `APP_CORS_ALLOWED_HEADERS`, `APP_SWAGGER_ENABLED` (default `false`; enables `/openapi.json` and Swagger UI), `APP_SWAGGER_BASE_PATH` (default `/swagger`), `APP_SWAGGER_SERVER_URL`, as well as per-database settings for `Main` (`DB_MAIN_*`) and `Analytics` (`DB_ANALYTICS_*`): `*_DRIVER` (`sqlite`, `postgres`, `pgx`, `sqlserver`, `mssql`), `*_DSN`, `*_READ_ONLY` (`true`, `false`), `*_MAX_OPEN_CONNS`, `*_MAX_IDLE_CONNS`, `*_MAX_LIFETIME`, and `*_MAX_IDLE_TIME`. The last two use Go duration strings; `0` means unlimited.
...
This repository is a Go foundation for HTTP services. Gin handles HTTP routing, `samber/do/v2` owns runtime dependency injection, zerolog owns application logging, and database connections are opened through `database/sql` using drivers for SQLite (`modernc.org/sqlite`), PostgreSQL (`github.com/jackc/pgx/v5/stdlib`), and MSSQL (`github.com/microsoft/go-mssqldb`).
...
- `database`: opens configured database connections (`Main` and `Analytics`) supporting SQLite, PostgreSQL, and MSSQL, provides `*database.Databases`, named `*sql.DB`, and `map[string]*sql.DB`, pings them during initialization, and closes them during destruction.
